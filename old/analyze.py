...
import simstat,simcalc,simhist,simprof,simplot,simfile,mmlbuildgal,mmlgadget,mmlscf,mmlgalfit,runsheet,mml_pNbody
...
def analyze(simstr,method=None,ftype=None,ptype=None,overwrite=None,verbose=None,shortlist=False,
    ...
    if method in methdict['stat']+methdict['calc']+methdict['hist']+methdict['plotsnap']:
        for isnapfile in snaplist:
            ...
            # Write out info on what was loaded
#            icalcfile=simcalc.snapfiles(simstr,'calcsnap',fext=fext)['file']
#            if inb==None:
#                if verbose: mmlio.verbose('Did not need to load snapshot:')
#                if verbose: print '    '+isnapfile
#            if calcdict==None:
#                if verbose: mmlio.verbose('Did not need to load calcdict:')
#                if verbose: print '    '+icalcfile
...
def snaphist(simstr,loadkeys,flagdict=None,inb=None,method=None,histdict=None,limdict=None,
    ...
    if histdict==None: 
        ...
        else                     : histdict=simfile.askfpar('hist',hmeth,askuser=True,fparfile=fparfile)
    # Create histogram/plot
    if not setflags:
        # Histogram
        if pmeth==None:
            # Only continue if overwrite selected or file does not exist
            histfile=simhist.snapfiles(simstr,histmethod,fext=loadkeys['fext'],**histdict)['file']
        ...
        # Plot
            ...
            # Only continue if overwrite selected or file does not exist
            plotfile=simhist.snapfiles(simstr,hmeth,fext=loadkeys['fext'],plot=True,plotmeth=pmeth,**histdict)['file']
            if flagdict[histmethod] or not os.path.isfile(plotfile):
                ...
                histdict,limdict,inb=simhist.plothist(simstr,hmeth,pmeth,histdict=histdict,limdict=limdict,
                                                      overwrite=flagdict[histmethod],askuser=False,
...
def snapstat(simstr,loadkeys,flagdict=None,inb=None,statdict=None,fparfile=None,
    ...
    for istat in statlist:
        ...
        if istat+'_fpar' not in flagdict: flagdict[istat+'_fpar']=simfile.askfpar('stat',istat,askuser=True,
                                                                                  fpar_deft=ifpar_deft,fparfile=fparfile)
...
#        statfile=filedict['stat']['stat_'+istat]
        ...
        if not setflags:
            ...
            if loadkeys['fname'] not in statdict[istat]['fname'] or flagdict[istat]:
                ...
                if verbose: mmlio.verbose('Added {} statistics to file.'.format(istat))
    # Return output
