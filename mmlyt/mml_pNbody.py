...
class mmlNbodyDefault:
  ...
  def __init__(self,p_name=None,pos=None,vel=None,mass=None,num=None,tpe=None,ftype=None,status='old',
    ...
    if ftype=='gadget':
      # Get unitsfile
      if isinstance(unitsfile,str):
        if os.path.isfile(unitsfile): self.read_gparams(unitsfile)
        else: unitsfile=None
      else:
        unitsfile=None
      # Get makefile
      if isinstance(makefile,str):
        if os.path.isfile(makefile): self.read_makefile(makefile)
        else: makefile=None
      else:
        makefile=None
    # SCF files
      ...
      if isinstance(unitsfile,str):
        if not os.path.isfile(unitsfile): unitsfile=None
      else:
        unitsfile=None
    else:
      pass
...
#      var=v/np.ma.masked_equal(r,0)
      ...
      try:
        ...
        var=None
    if var==None: raise Exception('No such sim attribute: {}'.format(mode))
    return var
