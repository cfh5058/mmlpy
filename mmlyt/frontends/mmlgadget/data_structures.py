...
from .fields import \
    MMLGadgetFieldInfo, \
    add_stream_field, \
    KnownMMLGadgetFields

class MMLGadgetGrid(AMRGridPatch):
    """
    ...
    def __init__(self, id, hierarchy):
        """
        Returns an instance of MMLGadgetGrid with *id*, associated with *filename*
        and *hierarchy*.
        """
    ...
    def __repr__(self):
        return "MMLGadgetGrid_%04i" % (self.id)
...
class MMLGadgetHierarchy(AMRHierarchy):
...
class MMLGadgetStaticOutput(StaticOutput):
    _hierarchy_class = MMLGadgetHierarchy
    _fieldinfo_fallback = MMLGadgetFieldInfo
    _fieldinfo_known = KnownMMLGadgetFields
    _data_style = 'stream'
...
class MMLGadgetDictFieldHandler(dict):
...
def load_uniform_grid(data, domain_dimensions, sim_unit_to_cm, bbox=None,
    ...
    :class:`~yt.frontends.stream.data_structures.MMLGadgetHandler`.
    ...
    spf = MMLGadgetStaticOutput(handler)
