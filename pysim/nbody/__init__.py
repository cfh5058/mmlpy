...
# Import subpackages
from . import util, filt, array, family, snapshot,  tipsy, gadget, gadgethdf, ramses, grafic, analysis, halo, derived, bridge, gravity, sph, nchilada, buildgal, scf
...
_snap_classes = [gadgethdf.GadgetHDFSnap, nchilada.NchiladaSnap, gadget.GadgetSnap, tipsy.TipsySnap, ramses.RamsesSnap, grafic.GrafICSnap,
                 buildgal.BuildgalSnap, scf.ScfSnap]
...
def load(filename, filetype=None, *args, **kwargs) :
    """Loads a file using the appropriate class, returning a SimSnap
    instance."""

    if filetype in config['snap-class-priority']: 
        ftypelist=[filetype]
    else: 
        ftypelist=config['snap-class-priority']
        
    for c in ftypelist :
        if c._can_load(filename) :
            if config['verbose'] : print>>sys.stderr, "Loading {} using backend {}".format(filename,str(c))
