...
class SimArray(np.ndarray) :
    ...
    def in_unitsys(self, new_unitsys) :
        for idx in range(len(new_unitsys)): new_unitsys[idx]=units.Unit(new_unitsys[idx])
        dimensions = self.units
        if dimensions==1 or dimensions==units.NoUnit(): return self
        d = dimensions.dimensional_project(new_unitsys)
        new_unit = reduce(lambda x,y: x*y, [a**b for a,b in zip(new_unitsys,d)])
        return self.in_units(new_unit)
