...
def intersect_slices(s1, s2, array_length=None) :
    ...
    # Array
    elif isinstance(s1,np.ndarray) or isinstance(s2,np.ndarray):
        for ss in [s1,s2]:
            if isinstance(ss,np.ndarray) and np.issubdtype(np.bool,ss):
                array_length=len(ss)
        if isinstance(s1,slice) or not np.issubdtype(np.bool,s1.dtype):
            assert array_length is not None
            s1arr=np.zeros(array_length,dtype=bool)
            s1arr[s1]=True
        else: s1arr=s1
        if isinstance(s2,slice) or not np.issubdtype(np.bool,s2.dtype):
            assert array_length is not None
            s2arr=np.zeros(array_length,dtype=bool)
            s2arr[s2]=True
        else: s2arr=s2
        return np.logical_and(s1arr,s2arr)

    # Error
    else: raise Exception('Unknown slice type {} or {}'.format(type(s1),type(s2)))
...
def relative_slice(s_relative_to, s) :
    ...
    if isinstance(s_relative_to,slice) and isinstance(s,slice):
    ...
    # Array
    elif isinstance(s_relative_to,np.ndarray) or isinstance(s,np.ndarray):
        for ss in [s_relative_to,s]:
            if isinstance(ss,np.ndarray) and np.issubdtype(np.bool,ss):
                array_length=len(ss)
        if isinstance(s_relative_to,slice) or not np.issubdtype(np.bool,s_relative_to.dtype):
            assert array_length is not None
            sarr_relative_to=np.zeros(array_length,dtype=bool)
            sarr_relative_to[s_relative_to]=True
        else: sarr_relative_to=s_relative_to
        if isinstance(s,slice) or not np.issubdtype(np.bool,s.dtype):
            assert array_length is not None
            sarr=np.zeros(array_length,dtype=bool)
            sarr[s]=True
        else: sarr=s
        if not np.all(sarr_relative_to[sarr]):
            raise ValueError, "Incompatible slices"
        return sarr[sarr_relative_to]
    # Error
    else: raise Exception('Unknown slice type {} or {}'.format(type(s_relative_to),type(s)))
...
def chained_slice(s1,s2) :
    ...
    # Array
        ...
        if isinstance(s2,np.ndarray) and np.issubdtype(np.bool,s2):
            array_length=len(s2)
        if isinstance(s1,slice) or not np.issubdtype(np.bool,s1.dtype):
            assert array_length is not None
            s1arr=np.zeros(array_length,dtype=bool)
        ...
        if len(s2)!=s1.sum():
            raise ValueError, "Incompatible slices"
        s3arr = np.zeros(array_length,dtype=bool)
        s3arr[np.where(s1arr)[0][s2arr]]=True
        return s3arr
    # Error
    else: raise Exception('Unknown slice type {} or {}'.format(type(s1),type(s2)))
